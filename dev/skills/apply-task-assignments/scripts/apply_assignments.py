...
def parse_decisions(board_path: Path) -> tuple[str, list[Decision]]:
    ...
    for section in ASSIGNMENT_SECTIONS:
        ...
        if "task" not in header_map:
            continue
        owner_col = next((col for col in OWNER_COLUMNS if col in header_map), None)
...
def build_move_plans(dev: Path, decisions: list[Decision], team: dict[str, str]) -> list[MovePlan]:
    ...
    for decision in decisions:
        ...
        normalized_link = decision.source_link.strip()
        if not normalized_link.startswith(f"pool/{decision.uuid8}/"):
            raise ValueError(f"task {decision.uuid8} must point to pool/{decision.uuid8}/TASK.md, got {normalized_link}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not assigned_by:
        raise SystemExit("no leader found; pass --assigned-by")
    if assigned_by not in team:
        raise SystemExit(f"assigned_by is not in TEAM.md: {assigned_by}")
