...
---
继续用 dev/ **团队并发**开发 OS 干活（范式源 = Multi-Dev-Os）。**folder 全 per-dev 化,读任何一类要遍历 `{type}/*/` 聚合;导航 map(`DEVMAP.md`/各 `_NAV.md`)只定位、实时看原文 + 代码。**

1. **认身份**：读 `dev/.identity`（你是谁 = developer_id）+ `dev/TEAM.md`（全员 + 你的 role:leader/admin/developer）。
2. **读地基**：`dev/GOAL.md`（终态）+ 你的 `dev/state/{你}/state.md`（现状）+ `dev/board/{你}/board.md`（你的卡）+ `dev/RULES.md` + `dev/RULES.project.md`（本项目红线）+ 决策（遍历 `dev/decisions/*/`，先看 `dev/DEVMAP.md`/各 `_NAV.md` 定位再读原文）。重资料 `research/archive/` read-on-demand。
